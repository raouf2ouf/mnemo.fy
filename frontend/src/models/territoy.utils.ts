...
export function contains(t1: Shape, t2: Shape): boolean {
  return (
    t1.min.x < t2.min.x &&
    t1.min.y < t2.min.y &&
    t1.max.x > t2.max.x &&
...
export function checkOverlap(shapes: Shape[]): Shape[] {
  const nonOverlap: Shape[] = [];
  shapes.sort((a, b) => a.min.x - b.min.x);
  for (const shape of shapes) {
    if (!shape.contained) {
      for (const otherShape of shapes) {
        if (contains(shape, otherShape)) {
          shape.inverse.push(otherShape.points);
          otherShape.contained = true;
        }
      }
      nonOverlap.push(shape);
    }
  }
  return nonOverlap;
}

export function buildShapes(pointsMap: Map<string, string>): Shape[] {
  const shapes: Shape[] = [];
  ...
  while (pointsMap.size > 0) {
    if (!startingPt) {
      ...
      shape.points.push(stringToPoint(startingPt!));
      shapes.push(shape);
    }
    ...
    if (nextPt === undefined || nextPt === startingPt) {
      ...
      pointsMap.delete(currentPt!);
      shape!.points.push(stringToPoint(nextPt));
      currentPt = nextPt;
    }
  }
  for (const shape of shapes) {
    const { min, max } = computeMinMax(shape.points);
    shape.min = min;
    shape.max = max;
  }
  return checkOverlap(shapes);
}

/* Algorithm for titlePosition&TitleSize from: https://github.com/mapbox/polylabel */
const PRECISION = 100.0;

const getCentroidCell = (shape: Shape) => {
  let area = 0;
  let x = 0;
  let y = 0;
...
const pointToPolygonDist = (x: number, y: number, shape: Shape) => {
  let inside = false;
...
class Cell {
  ...
  distance: number;
  max: number;
  constructor(x: number, y: number, radius: number, shape: Shape) {
    this.x = x;
    this.y = y;
    this.radius = radius;
...
export function computeTitlePositionSize(t: Shape): {
  titlePosition: Point;
  titleSize: number;
} {
